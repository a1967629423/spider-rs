...
impl BoxContextID {
    pub fn new<T>(id:T) -> Self where T:ContextID + Send + Sync + 'static {
...
impl BoxError {
    pub fn new<T>(err:T) -> Self where T:std::error::Error + Send + 'static {
        Self(Box::new(err))
    }
...
impl BoxContentType {
    pub fn new<T>(content:T) -> Self where T:ContentType + Sync + Send + 'static {
